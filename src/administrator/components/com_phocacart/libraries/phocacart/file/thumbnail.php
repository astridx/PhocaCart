...
class PhocacartFileThumbnail
{
	...
	public static function deleteFileThumbnail($filename, $small=0, $medium=0, $large=0, $manager)
	{
		...
		if ($small == 1)
		{
			$fileNameThumbS = self::getThumbnailName($filename, 'small', $manager);
		...
		if ($medium == 1)
		{
			$fileNameThumbM = self::getThumbnailName($filename, 'medium', $manager);
		...
		if ($large == 1)
		{
			$fileNameThumbL = self::getThumbnailName($filename, 'large', $manager);
		...
		switch ($ext)
		{
			...
			case 'webp':
				...
				if ($onlyThumbnailInfo == 0)
				{
					...
					$creatingFolder = self::createThumbnailFolder($file['path_without_file_name_original'], $file['path_without_file_name_thumb'], $errorMsg);
					...
					if (JFolder::exists($file['path_without_file_name_thumb']))
					{
						...
						if ($small == 1)
						{
							self::createFileThumbnail($file['name_original_abs'], $thumbNameS->abs, 'small', $frontUpload, $manager, $errorMsgS);
						}
						else
						{
						...
						if ($medium == 1)
						{
							self::createFileThumbnail($file['name_original_abs'], $thumbNameM->abs, 'medium', $frontUpload, $manager, $errorMsgM);
						}
						else
						{
						...
						if ($large == 1)
						{
							self::createFileThumbnail($file['name_original_abs'], $thumbNameL->abs, 'large', $frontUpload, $manager, $errorMsgL);
						}
						else
						{
						...
						elseif ($errorMsgS == '' && $errorMsgM == '' && $errorMsgL == '')
						{
							$returnFrontMessage = 'Success';
						}
						elseif ($errorMsgS == '' && $errorMsgM == '' && $errorMsgL == 'ThumbnailExists')
						{
							$returnFrontMessage = 'Success';
						}
						elseif ($errorMsgS == '' && $errorMsgM == 'ThumbnailExists' && $errorMsgL == 'ThumbnailExists')
						{
							$returnFrontMessage = 'Success';
						}
						elseif ($errorMsgS == '' && $errorMsgM == 'ThumbnailExists' && $errorMsgL == '')
						{
							$returnFrontMessage = 'Success';
						}
						elseif ($errorMsgS == 'ThumbnailExists' && $errorMsgM == 'ThumbnailExists' && $errorMsgL == '')
						{
							$returnFrontMessage = 'Success';
						}
						elseif ($errorMsgS == 'ThumbnailExists' && $errorMsgM == '' && $errorMsgL == '')
						{
							$returnFrontMessage = 'Success';
						}
						elseif ($errorMsgS == 'ThumbnailExists' && $errorMsgM == '' && $errorMsgL == 'ThumbnailExists')
						{
							$returnFrontMessage = 'Success';
						}
						elseif ($errorMsgS == 'ThumbnailExists' && $errorMsgM == 'ThumbnailExists' && $errorMsgL == 'ThumbnailExists')
						{
						...
						elseif ($returnFrontMessage == 'SuccessThumbnailExists')
						{
	...
	public static function createFileThumbnail($fileOriginal, $fileThumbnail, $size, $frontUpload=0, $manager, &$errorMsg)
	{
		...
		if ($enable_thumb_creation == 1)
		{
			$fileResize	= self::getThumbnailResize($size);
