...
class PhocacartOrder
{
	...
	public function saveOrderMain($data)
	{
		...
		elseif (empty($shippingMethods) && PhocacartPos::isPos())
		{
			// IS OK - shipping method was not selected but we are in POS
		}
		elseif (empty($shippingMethods) && $shippingNotUsed)
		{
		...
		elseif ($shippingNotFoundAllowProceed)
		{
			// IS OK
			$shippingId = 0;
		}
		...
		elseif (empty($paymentMethods) && PhocacartPos::isPos())
		{
			// IS OK
		}
		elseif (empty($paymentMethods) && $paymentNotUsed)
		{
			// IS OK
			$paymentId = 0;
		}
		elseif ($paymentNotFoundAllowProceed)
		{
			// IS OK
			$paymentId = 0;
		}
		...
		if ($row->id > 0)
		{
			...
			elseif (isset($address[1]->type) && $address[1]->type == 0)
			{
				$this->cleanTable('phocacart_order_users', $row->id);
			...
			elseif (isset($address[0]->type) && $address[0]->type == 1)
			{
				$this->saveOrderUsers($address[0], $row->id);
			}
	...
	public function saveOrderBilling($id, $date)
	{
		...
		$d['order_number']		    = self::getOrderNumber($id, $date, false, $d['order_number_id']);
		$d['receipt_number']	    = self::getReceiptNumber($id, $date, false, $d['receipt_number_id']);
		$d['invoice_number']	    = self::getInvoiceNumber($id, $date, false, $d['invoice_number_id']);
		...
		$d['invoice_prn']		    = self::getPaymentReferenceNumber($id, $date, false, $idNumberA);
		$d['date']				    = $date;
		$d['invoice_date']		    = $date;
		$d['invoice_due_date']	    = self::getInvoiceDueDate($id, $date);
