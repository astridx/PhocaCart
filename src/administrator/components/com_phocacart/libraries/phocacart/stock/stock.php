...
class PhocacartStock
{
	...
	public static function getStockStatusOutput($stockStatus)
	{
		...
		elseif (!$stockStatus['stock_status'] && $stockStatus['stock_count'] !== false)
		{
			$o .= $stockStatus['stock_count'];
		}
		elseif ($stockStatus['stock_status'] /* && $stockStatus['status_display_count'] == 0*/)
		{
	...
	public static function getStockItemsChangedByAttributes(&$stockStatus, $attributes, $item, $ajax = 0)
	{
		...
		elseif ($item->stock_calculation == 2)
		{
		...
		$stockStatus		= self::getStockStatus((int) $stock, (int) $item->min_quantity, (int) $item->min_multiple_quantity, (int) $item->stockstatus_a_id,  (int) $item->stockstatus_n_id);
