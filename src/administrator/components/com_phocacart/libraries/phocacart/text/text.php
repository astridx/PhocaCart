...
class PhocacartText
{
	...
	public static function completeTextFormFields($body, $bas, $type = 1)
	{
		...
		if (!empty($bas))
		{
			...
			if (isset($bas['order_id']))
			{
				unset($bas['order_id']);
			}

			if (isset($bas['user_address_id']))
			{
				unset($bas['user_address_id']);
			}

			if (isset($bas['user_token']))
			{
				unset($bas['user_token']);
			}

			if (isset($bas['user_groups']))
			{
				unset($bas['user_groups']);
			}
