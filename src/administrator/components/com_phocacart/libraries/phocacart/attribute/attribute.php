...
return urlencode(substr($value, 0, self::getAttributeLength($type)));
			...
			foreach ($attributes as $k => $v)
			{
				...
				if (!empty($v))
				{
					foreach ($v as $k2 => $v2)
					{
						if ((int) $k > 0 && (int) $k2 > 0)
						{
							$attrib = self::getAttributeValue((int) $k2, (int) $k);
	...
	public static function checkRequiredAttributes($id, $attributes)
	{
		...
		$requiredAttributes = self::getAllRequiredAttributesByProduct($id);
								...
								if (!empty($v3))
								{
									foreach ($v3 as $k4 => $v4)
									{
										...
										if (isset($v2['type']) && ($v2['type'] == 7 || $v2['type'] == 8 || $v2['type'] == 9))
										{
											...
											elseif (!is_array($v4) && urldecode($v4 != ''))
											{
												// Order product - we found value in order of products - OK
												$passAttribute[$k3] = 1;
